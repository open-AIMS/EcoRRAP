...
# Update the model reference if there are enough (5 or more) scale bars (scale the model), or throw a warning.
if len(chunk.scalebars) >4:
    chunk.updateTransform()
else:
    Metashape.app.messageBox("WARNING: There are less than 5 scale bars.")
...
for marker in chunk.markers:
        ...
        if len(marker.projections) < 1:
            Metashape.app.messageBox("You've added data for at least one marker that hasn't been detected. Proceeding, but check the reference pane")
            chunk.remove(marker)
...
# We would like to return an error if the scale bars have a lot of error. More than 2cm is bad.
